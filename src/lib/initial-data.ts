...
export interface AreaPositionOffset {
  ...
  // перетаскивание (handleAreaDragEnd) запрашивает явное подтверждение через
  // confirm(). Поворот отдельно не защищён: текущий (возможно, подстроенный)
  // поворот уходит в areas.json как базовый прямо при экспорте
  // (handleExportAreas) — экспорт и так только dev-действие.
  verified?: boolean;
}
